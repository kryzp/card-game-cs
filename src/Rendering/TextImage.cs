namespace MauMau.Rendering
{
	public class TextImage
	{
		...
		public TextImage(string text, ConsoleColor col)
			: this()
        {
			DrawText(text, col);
        }
		...
		public void Draw(Coordinates coords, Func<int, int, ColouredChar, bool> shouldDraw, bool overrideEverything = false)
		{
			for (int y = 0; y < Chars.Count; y++)
			{
				for (int x = 0; x < Chars[y].Count; x++)
				{
					...
					if ((c.Char != ' ' || overrideEverything) && shouldDraw(x, y, c))
						c.Write(x + coords.X, y + coords.Y);
				}
			}
		}
